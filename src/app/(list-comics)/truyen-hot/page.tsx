...
import RenderTrendingComics from '@/components/listComic/RenderTrendingComics'
...
const fetchDataHotComics = async (page?: string) => {
    try {
        const res = await fetch(`https://comics-api.vercel.app/trending-comics${page ? `?page=${page}` : ''}`) // Call API lấy truyện full trending
        const data = res.json()
        return data
    } catch (error) {
        console.log('Error');
...
export async function generateMetadata() {
    ...
    return {
        title: `Truyện Hot | Truyện tranh hay`,
...
const HotComics: FC<completedProps> = async ({ searchParams }) => {
    const { comics, current_page, total_pages } = await fetchDataHotComics(searchParams.page)
            ...
            <div className='container flex items-start justify-center relative flex-wrap !md:px-2 !px-0'>
                <div className='relative rounded-md md:mt-5 mt-3 w-full bg-[#f6f3ee] md:p-5 p-3'>
                    <div className='flex items-center font-semibold sm:text-lg text-xs'>
                        ...
                        <Link href="/the-loai?type=all" className='hover:underline text-slate-700'>Truyện Hot</Link>
                        <div className="text-sm mx-2 "><BsChevronRight /></div>
                ...
                <div className='w-full relative'>
                    <div className='relative bg-[#f6f3ee] rounded-md md:py-5 md:px-3 md:mt-5 xl:mb-12 mb-5'>
                        <RenderTrendingComics data={comics} total_pages={total_pages} current_page={current_page} params={searchParams.page} />
                    </div>
                </div>
            </div>
        </section>
